...
7. **홈 배너 로딩 깜빡임 제거**: 노션 배너(`/api/banners`)를 비동기로 불러오는 동안 기본 배너(파란 그라데이션+로고)가 먼저 보였다가 로딩 완료 시 실제 배너로 교체돼서, 느린 네트워크에서 전환이 눈에 띄던 문제 제보. `.banner`에 `is-loading` 클래스(`visibility: hidden`)를 기본으로 걸어두고, `loadBanners()`가 노션 배너 유무를 확정한 뒤(있으면 그것만, 없으면 기본 배너 그대로) 클래스를 제거해 한 번에 드러나도록 수정 — 자리는 그대로 유지해 레이아웃 밀림 없음 (커밋 `f5e2954`, `sw.js` 캐시 v31→v32)
