...
기존 홈 화면(정적 히어로 배너 + 단순 지역 그리드)을 여기어때류 여행 앱 톤으로 다듬음:

1. **스플래시 화면**: `index.html` 최상단에 `#splash` 오버레이 추가, Primary 컬러 배경에 메인 로고를 팝 애니메이션으로 노출. `sessionStorage.splashShown`으로 세션당 1회만 노출(2초 후 페이드아웃), 재방문/새로고침 시 바로 스킵
2. **히어로 배너 → 슬라이더**: 기존 "로고+태그라인" 고정 배너를 3장 슬라이드(로고/300+ 크리에이터/99곳 전국)로 교체. `js/app.js`의 `initHeroSlider()`가 3초 간격 자동 전환 + 하단 도트 인디케이터 담당
3. **지역 그리드 아이콘화**: 10개 지역 각각에 이모지 아이콘(`REGION_EMOJI` 매핑, 서울 🗼·제주 🌴 등) 추가, 카드에 그라데이션 배경·그림자·둥근 모서리 적용해 입체감 부여
4. **하단 탭바 아이콘화**: 홈🏠/지도🗺️/찜❤️/내정보👤 이모지를 텍스트 라벨 앞에 추가. index/map/place 세 페이지 모두 동일하게 반영 (탭바 마크업이 페이지마다 중복되어 있어 3곳 각각 수정)
5. **장소 카드 스타일 강화**: 카드 그림자·둥근 모서리 확대, 탭 시 눌리는 `:active` 스케일 인터랙션 추가, 썸네일 크기 확대(88px→108px)
6. **검증**: `wrangler dev` 로컬 서버로 브라우저에서 스플래시 노출→페이드아웃, 히어로 슬라이더 자동 전환·도트 동기화, 지역/탭바 아이콘 렌더링을 index/map/place 세 페이지 모두 확인. 콘솔 에러 없음. 커밋 `18f23ba`로 반영, origin/main에 푸시 완료

**남은 작업(디자인 관련, 우선순위 낮은 순 아님)**:
- 카테고리 태그 필터 칩도 아이콘화 여부 검토
- 찜❤️/내정보👤 탭은 아직 실제 페이지 없이 `#` 링크 (3-5 참고)
