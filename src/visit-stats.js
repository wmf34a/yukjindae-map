...
// Analytics Engine 을 붙이기 전의 누적. 여기에는 표본 배수를 곱하면 안 된다
// (그렇게 곱했다가 267명이 1,335명으로 보였다).
//
// ── 이 숫자는 실측이 아니라 추정이다 ──
//
// 오픈일(2026-09-01) 13시 30분에 KV 하루 쓰기 한도가 차면서 집계가 멈췄다. 그때까지
// 센 것이 267명이고, 그 뒤 열 시간은 아무것도 세지 못했다. 267을 그대로 두면 실제보다
// 한참 적다.
//
// 그래서 Cloudflare 요청 로그로 되짚었다. 그날 실사용자 요청이 약 102,500건이고, 홈을
// 한 번 여는 데 우리 서버로 23개가 나간다(직접 측정). 한 사람이 아주 오래 머물러
// 80개씩 썼다고 가정해도 1,281명이고, 보통 수준으로 둘러봤다면 1,700~2,500명이다.
// 그 범위의 가장 아래를 택했다 — 어떤 가정을 써도 이보다는 많았다고 말할 수 있는 값.
//
// 인스타 릴스를 올린 날이고 그날 팔로워가 1,100명 늘었다는 것도 같은 방향을 가리킨다.
//
// 정확한 수는 복구할 수 없다. 이 값을 "그날 방문자 수"로 인용하면 안 된다.
export const TOTAL_BASELINE = 1300;
// 그 기준선을 쌓은 마지막 날. 이 날까지는 위 추정치가, 다음 날부터는 Analytics Engine
// 이 실제로 센 값이 더해진다.
