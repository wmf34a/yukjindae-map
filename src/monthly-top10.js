...
const SEASON_HINTS = {
  ...
  9: "초가을. 더위가 꺾인다. 야외 활동과 체험을 우대한다. 아직 단풍철은 아니다.",
  10: "가을. 야외에 가장 좋은 시기. 단풍이 북쪽·산간부터 물들기 시작한다.",
  11: "늦가을. 남부는 단풍 절정이고 중부는 끝물이다. 후반으로 갈수록 쌀쌀해져 실내 비중을 늘린다.",
...
// 단풍 절정은 한 달 안에서도 지역마다 3~4주씩 어긋난다. 강원 산간은 10월
// 중순, 중부는 10월 하순~11월 초, 남부는 11월 초순이다. "10~11월"로 뭉뚱그리면
// 9월에는 이르고 11월 말에는 늦은 추천이 나온다.
//
// 표시(autumnSpot)는 짐작이 아니라 블로그에서 "○○ 단풍" 언급을 세어 붙인 것이다.
const AUTUMN_PEAK = {
  강원도: { 10: "절정", 11: "끝물" },
  경기북부: { 10: "절정", 11: "절정" },
  서울강북: { 10: "들기 시작", 11: "절정" },
...
export function autumnRule(month, region) {
  const stage = AUTUMN_PEAK[region]?.[month];
  if (!stage) return "";
  if (stage === "절정") {
    return `- 이 지역은 지금이 단풍 절정이다. "가을: 단풍 명소" 표시가 있는 곳을 가장 위로 올린다.`;
  }
  if (stage === "들기 시작") {
    return `- 이 지역은 단풍이 물들기 시작하는 때다. "가을: 단풍 명소" 표시가 있는 곳에 가산점을 준다.`;
  }
  return `- 이 지역은 단풍이 끝물이다. "가을: 단풍 명소" 표시는 가산점 정도로만 본다.`;
}
...
export function buildPrompt({ monthKey, region, candidates }) {
  ...
  return [
    ...
    ...(autumnRule(month, region) ? [autumnRule(month, region)] : []),
