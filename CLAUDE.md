...
`withEdgeCache` 는 만료돼도 옛 응답을 먼저 돌려주고 갱신은 `ctx.waitUntil` 로
뒤에서 돌린다. 여기에 걸린 함정이 몇 개 있다.

- **만료 직후 요청마다 갱신을 띄우면 노션을 동시에 두드린다.** 서로 다른 DB 두
  곳이 1초 안에 같이 타임아웃 나고, 그 갱신들이 `waitUntil` 유예를 넘겨 통째로
  취소되는 일이 하룻밤에 다섯 번 있었다. 갱신이 한 번도 안 붙으면 보관 기간이
  끝나는 순간 캐시가 비고, 그때 들어온 사람이 7.7초를 기다린다(실측).
  그래서 갱신을 시작하기 전에 들고 있던 응답의 `CACHED_AT` 만 앞당겨 다시 넣어
  뒤따르는 요청이 갱신을 또 띄우지 않게 한다.
- **그 표시가 무한정 연장되면 옛 값을 언제까지나 보여주게 된다.** 원본을 실제로
  받은 시각은 `FETCHED_AT` 에 따로 적고, 보관 기간을 넘겼으면 표시하지 않는다.
- **조건부 요청에는 `cache.match` 가 304 를 준다.** 그걸 표시로 다시 넣으면
  Workers 가 `Ignoring attempt to Cache.put() a 304` 를 찍고 넣기를 버린다.
  `cached.status === 200` 일 때만 표시한다.
- **엣지 캐시는 콜로마다 따로다.** 크론의 `warmCaches` 가 예열하는 것은 그
  요청이 닿은 콜로 하나뿐이라, 다른 콜로로 들어온 사람은 그대로 콜드 미스다.
  예열로는 못 덮으므로 프론트의 last-good 폴백(`public/js/util.js`)이 실질적인
  방어선이다 — 재방문자는 목록이 비지 않는다. 첫 방문자는 방법이 없다.

시간 예산은 이렇게 맞물려 있다. 바꿀 때 셋을 같이 봐야 한다.

```
노션 읽기 타임아웃 4초 (READ_TIMEOUT_MS) + 백오프 0.4초 + 재시도 1회 = 최악 8.4초
프론트 포기                                          10초 (FETCH_TIMEOUT_MS)
```

읽기 타임아웃을 늘리면 재시도까지의 최악값이 프론트 창을 넘겨, 느린 요청이
폴백으로 떨어지기만 하고 재시도 이득은 못 본다.
...
### 수유실 명부는 User-Agent 가 없으면 403 이다

`sooyusil.com` (수유정보 알리미, 전국 3,000곳) 은 인증을 안 본다 — 틀린 키로도
200 을 준다. 대신 **User-Agent 가 비면 403** 이다. Workers 의 `fetch` 는 UA 를
붙이지 않고 나가서, 2026-09-05 주간 크론에서 열일곱 시·도가 한꺼번에 막혔다.
`SOOYUSIL_USER_AGENT` 를 헤더로 넣어 부른다.

이게 조용한 이유는 KV 가 명부를 14일 들고 있고 갱신 크론은 주 1회라서다. 한 번
막혀도 앱은 멀쩡해 보이고, 두 주 연속 막히면 그때 수유실이 통째로 사라진다.
403 로그를 보면 다음 주 크론 전에 고쳐야 한다.
...
## 방문 분석 (Analytics Engine)

`env.VISITS` 로 데이터셋 `yukjindae_visits` 에 쓴다. KV 카운터는 하루 쓰기 한도를
반나절에 태워서 쓸 수 없다.
...
curl -s "https://api.cloudflare.com/client/v4/accounts/$ACC/analytics_engine/sql" \
  -H "Authorization: Bearer $TOK" --data "SELECT ... FORMAT JSON"
```

**SQL 함정** — 여기서 시간을 많이 버렸다.

- 별칭에 한글을 쓰면 `Expected an identifier after AS` 로 죽는다. 영문 별칭만 쓴다
- **서브쿼리 별칭을 바깥에서 못 쓴다** (`unable to find type of column`). 묶음
  집계는 SQL 로 두 번 접지 말고, 한 번만 뽑아서 파이썬에서 접는 게 빠르다
- KST 는 `toHour(timestamp + INTERVAL '9' HOUR)`, 기간은 `NOW() - INTERVAL 'N' DAY`
- 보존 기간이 짧다. 며칠치밖에 안 남으므로 장기 추세는 따로 적어 둬야 한다

`blob5` 를 장소명으로 바꾸려면 `/api/places` 를 받아 `id.replace('-','')` 로 맞춘다.
목록에 없는 ID 는 축제 상세다.

### 2026-09-02~06 스냅샷

보존 기간이 짧아 숫자가 사라지므로 적어 둔다.

```
날짜        방문자   조회        사용자·하루당 조회수    재방문 (고유 1,975명)
09-02 수     989   1702        1회      61.6%        1일만    87.8%
09-03 목     453   1021        2회      16.4%        2일       9.0%
09-04 금     359    962        3~5회    14.5%        3일       2.5%
09-05 토     245    763        6~10회    4.7%        4일 이상  0.7%
09-06 일     251    797        11회+     2.7%
                               평균 2.3회            2일 이상 재방문 12.2%
```

- 9/2 정점 뒤 250명대에서 평평해졌고, **주말에 사람은 안 늘었다**
- 다만 주말에 더 깊이 본다: 조회 중 장소 상세가 평일 10~19% → 토 24.9%,
  지도가 3.0~7.2% → 토 11.7%. 홈에서 장소로 넘어가는 흐름은 작동한다
- 시간대는 아침 8~10시가 정점(10시 최대), 저녁 6~7시가 바닥, 밤 10시에 다시 오른다
- 많이 열린 장소 1위 물맑음수목원(25회/22명), 그다음 안성팜랜드·의왕 왕송호수·
  늘솔길공원(각 11회). **상위 20곳은 전부 사진이 있다**
- 9/2 는 화면 추적이 붙기 전이라 화면별 비중이 0 으로 나온다
